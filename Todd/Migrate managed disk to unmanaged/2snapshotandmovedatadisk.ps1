 #Variables for snapshot data disk
 ...
 #Snapshot Data Disk
 $dataDiskName = "MGD2UNMGD_DataDisk_0"
 $snapshotdatadiskName = "unmanageddatadisk"
 $disk = Get-AzureRmDisk -ResourceGroupName $resourceGroupName -DiskName $dataDiskName
 $snapshot = New-AzureRmSnapshotConfig -SourceUri $disk.Id -CreateOption Copy -Location $location 
 New-AzureRmSnapshot -Snapshot $snapshot -SnapshotName $snapshotdatadiskName -ResourceGroupName $resourceGroupName
 ...
 $sas = Grant-AzureRmSnapshotAccess -ResourceGroupName $ResourceGroupName -SnapshotName $snapshotdatadiskName  -DurationInSecond $sasExpiryDuration -Access Read 
  
